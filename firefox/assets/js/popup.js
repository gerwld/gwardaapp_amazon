...
(()=>{"use strict";document.addEventListener("DOMContentLoaded",()=>{const e=chrome||browser,c=document.getElementById("lang_set");let t;e.storage.local.onChanged.addListener((e,l)=>{e.gpState&&e.gpState.newValue&&JSON.stringify({...e.gpState.newValue})!==t&&(t=JSON.stringify({...e.gpState.newValue}),o())});let n;function a(l){e.storage.sync.get("cookies_gal20",e=>{!0!==e?.cookies_gal20?(e=document.querySelector('[data-action="disabled"]'))?(document.querySelector("footer").remove(),e?.parentElement?.removeChild(e),e?.remove(),e?.innerHTML&&(e.innerHTML=""),document.querySelector("section.content").innerHTML=`
...
<ul>`):setTimeout(a,20):l&&location.reload()})}function o(){e.storage.local.get("gpState",e=>{let l=e.gpState||{};var t=document.querySelectorAll("input, select");if(t)for(let e=0;e<t.length;e++){var n=t[e];"checkbox"===n.type?n.checked=l[n.name]||!1:n.value=l[n.name]||""}c&&l.lang_set&&(c.value=l.lang_set)})}e.storage.sync.onChanged.addListener((e,l)=>{e.cookies_gal20&&e.cookies_gal20.newValue&&JSON.stringify({...e.cookies_gal20.newValue})!==n&&a(n=JSON.stringify({...e.cookies_gal20.newValue}))}),a(),o()})})();